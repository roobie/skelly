...
const valid = loadFixture('valid-rifle');
...
describe('resolve', () => {
  ...
  it('mates ports face to face', () => {
    ...
    expectVec(at(r, 'magazine', [0, -10, 0]), [-5, -12.5, 0]); // magazine hangs down
  });
  ...
  it('places the same whichever side of a connection comes first', () => {
    const flipped = variant('valid-rifle', (a) => {
  ...
  it('places the same when placing a rolled part from its own side', () => {
    const rolled = variant('valid-rifle', (a) => {
...
describe('resolve: structure issues', () => {
  const structure = (edit: Parameters<typeof variant>[1]) =>
    resolve(variant('valid-rifle', edit), gunDomain).issues.map((i) => i.message);
  ...
  it('reports unknown ports, bad slots and bad rolls', () => {
    expect(structure((a) => { a.connections[0] = { from: 'receiver.nose', to: 'barrel.rear' }; }))
      .toEqual(['Connection #0: part "receiver" (receiver) has no port "nose".']);
    expect(structure((a) => { a.connections[6] = { from: 'receiver.rail', slot: 7, to: 'sight.base' }; }))
      .toEqual(['Connection #6: slot 7 is outside receiver.rail (0–6).']);
    expect(structure((a) => { a.connections[6] = { from: 'receiver.rail', slot: 1, to: 'sight.base', roll: 45 }; }))
      .toEqual(['Connection #6: roll 45 is not a multiple of 90.']);
  });

  it('leaves parts that nothing connects unplaced', () => {
    const r = resolve(variant('valid-rifle', (a) => {
